...
MSGraph::MSGraph(DBManager *db, QObject *parent)
    : QObject{parent}
{
    this->db = db;
...
void MSGraph::authorizationCallbackReceived(const QVariantMap &data){
    int bp = 0;
    bp++;
}


void MSGraph::authenticate()
{
    m_authFlow->grant();
}

void MSGraph::SaveAgenda(Agenda *agenda){
...
void MSGraph::saveAgenda2(QJsonObject data){
    /*
                       QString eventID = item.value("id").toString();
                    GraphBufferItem *item = new GraphBufferItem(GraphBufferType::DELETE, "https://graph.microsoft.com/v1.0/me/events/"+ eventID, QJsonObject());
                    this->AddBuffer(item);
*/
    ...
    for(int i = 0; i < agenda->dayModel()->m_data.size(); i++){
        ...
        for(int y = 0; y < item->m_data.size(); y++){
            ...
            QJsonObject obj;
            if(disItem->type == appointmentType){
                if(disItem->m_appointment->type() != "Block"){
                obj.insert("subject", disItem->m_appointment->title() + " // " + disItem->m_appointment->jobNo() + " // " + disItem->m_appointment->jobTitle());
                QJsonObject body;
                body.insert("contentType", "HTML");
                body.insert("content", disItem->m_appointment->typeName() + " // " + disItem->m_appointment->description() + " // Created by Planner.FV");
                obj.insert("body", body);
                QJsonObject start;
                start.insert("timeZone", "UTC");
                QString dateString = disItem->m_planDate.toUTC().toString("yyyy-MM-ddTHH:mm:ss");
...
void MSGraph::newAgenda(QJsonObject data){
...
void MSGraph::AfterDelete(QJsonObject data){

}

void MSGraph::AddBuffer(GraphBufferItem *item){
    m.lock();
...
int MSGraph::itemsLeft() const
{
    return m_itemsLeft;
}

void MSGraph::setItemsLeft(int newItemsLeft)
{
...
void MSGraph::setAuthenticated(bool newAuthenticated)
{
